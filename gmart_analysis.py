...
def load_sales_data():
    df = pd.read_csv("Gmart_sales_data.csv")
    return df
...
def create_visualizations(df, monthly_sales, category_revenue, city_revenue, discount_analysis, customer_spending):
    ...
    axes[1, 1].axvline(customer_spending.mean(), color='red', linestyle='--', linewidth=2,
                       label=f'Mean: ${customer_spending.mean():.0f}')
    axes[1, 1].legend()
...
def generate_insights(df, category_revenue, city_revenue, customer_spending):
    ...
    print(f" -> Revenue Share: {top_category_share:.2f}%")
